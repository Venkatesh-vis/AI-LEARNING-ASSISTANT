...
export const reviewFlashCard = async(req, res, next) => {
    try {
        ...
        const flashCardSet = await FlashCard.findOne({"cards._id" : cardId, userId: req.user.id});
...
export const toggleStartFlashCard = async(req, res, next) => {
    try {
        ...
        return res.status(200).json({
            success: true,
            message: `FlashCard ${flashCardSet.cards[cardIndex].isStarted ? 'started' : 'unstarted'}`,
            data: flashCardSet
        })
    }
    catch (err) {
...
export const deleteFlashCardSet = async(req, res, next) => {
    try {
        const {id} = req.params
        const flashCardSet = await FlashCard.findOne({_id: id, userId: req.user.id})
        if (!flashCardSet) {
            return res.status(404).json({
                success: false,
                message: "FlashCard Set not found"
